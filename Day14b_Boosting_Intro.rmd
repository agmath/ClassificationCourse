...
Boosting is implemented in `{tidymodels}` using trees. This is a "model class" which is defined by the `boost_tree()` model specification. The model `recipe()` and `workflow()` are defined as with all other `{tidymodels}` model classes. That is, to create a *boosting ensemble* of trees, we run
...
Note that the parameters set to `tune()` are hyperparameters whose values will be learned during cross-validation.

...
Now that we have some idea how support vector classifiers work, we can use `{tidymodels}` or `{sklearn}` to fit and use these types of models just like we would any other class of model. I'll fit several support vector classifiers on slightly different *toy* data sets and plot the results for you to view. The data sets will differ by a single observation being moved around.
...
A support vector classifier is a model class. We first import `SVC` from `sklearn.svm` and then create an instance of the model constructor using:
...
Support vector machines have several tunable hyperparameters. Some of the most common are

+ `C`, which is a positive number associated with the *cost* of predicting an observation in violation of the margin (either within the margin or on the wrong side of the classifier altogether).
+ `kernel` determines the type of support vector machine being built. Its options are `linear`, `poly`, `rbf`, `sigmoid`, or `precomputed` -- the default is `rbf`.

  + `degree` is a positive number identifying the polynomial degree (for *polynomial* support vector machines).

You can see the full `{sklearn}` documentation for `SVC` [here](https://scikit-learn.org/stable/modules/generated/sklearn.svm.SVC.html#sklearn.svm.SVC).

***

## Summary

In this notebook we introduced the basics of support vector classifiers. The classifiers we looked at produced linear boundaries, but there are other versions of support vector classifiers which produce non-linear boundaries. Linear algebra is doing some heavy lifting behind the scenes here. Since that course is not a prerequisite for this one, we'll just mention that there exist linear support vector machines, polynomial support vector machines, and radial basis support vector machines. They are each implemented in `{tidymodels}` with the model specifications `svm_linear()`, `svm_poly()`, and `svm_radial()` respectively and in `{sklearn}` via `SVC()` and its `kernel` parameter. An example of how these different variations of support vector machines appears below on a different dataset.

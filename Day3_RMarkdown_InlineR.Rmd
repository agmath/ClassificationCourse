...
This notebook will remind you how to use and navigate R Markdown documents. In particular it will cover
...
-   `eval = TRUE` says that the code in the code cell should be evaluated. Note that `TRUE` is the default for this, but setting `eval = FALSE` can help you with troubleshooting broken code or to preven cells that take a long time to run from running during the knitting process. Just beware, if a code cell is not evaluated, then no later code can use results from the code cell, otherwise errors will be thrown.
...
> *This report was prepared using all available web traffic data over the past week. Over that time period we had 348 unique visitors to our site.*
...
> *This report was prepared using all available web traffic data over the past week. Over that time period we had `` ` r site_traffic %>% nrow()` `` unique visitors to our site.*
...
Inline R is great, but writing lengthy commands inline is unweildy, difficult to troubleshoot, and difficult to read. For example, the following example sentence includes inline code required to extract a model coefficient from a fitted model.
...
Try what you've learned by working with our Kaggle InClass competition data on property price ranges in an R Markdown notebook. If you already worked with the competition data back on Day 2, you should revisit that notebook and identify opportunities to utilize inline R code, otherwise you should start a new R Markdown notebook. Be sure to keep your notebook in the directory corresponding to your GitHub repository that is being managed by RStudio -- don't forget to `Pull -> Commit -> Push` your changes to your GitHub repository when you are done.

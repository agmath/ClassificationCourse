...
For example, we might have a record corresponding to $\displaystyle{\vec{z} = \left[\begin{array}{c} 0.2\\ 0.45\\ 0.61\\ 0.08\right]}$. Then we'll have $\displaystyle{\sum_{j = 1} ^{4}{e^{z_k}} \approx 5.71}$. This will give $\displaystyle{\sigma\left(\vec{z}\right) = \left[\begin{array}{c} 0.2139\\ 0.2747\\ 0.3223\\ 0.1897\right]}$. The class membership probabilities output by softmax can be visualized in the bar plot below.

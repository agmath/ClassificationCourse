...
Using `{tidymodels}` we can use regular expressions in multiple ways. We can

+ tokenize words and then filter the results using a regular expression and the `str_detect()` function.
...
Perhaps we wanted all of the hashtags appearing in each tweet. Below is one way to do that -- it seems like Joe Biden  but I am sure that there is a more efficient way.

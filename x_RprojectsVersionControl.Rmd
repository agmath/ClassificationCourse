...
+ Navigate to our [course webpage](https://agmath.github.io/ClassificationCourse.html) and download the `*.Rmd` file for either *new* students or *returning* students. 

  + The difference between the two documents is the amount of guidance I am providing. If you remember the basics about piping chains of commands together in R, then use the *returning* student version. Otherwise, choose the *new* student version.
  
+ Open the `*.Rmd` document in RStudio and use `File -> Save As` to save the file in the directory corresponding to your R Project associated with the GitHub repository you made earlier.
...
Now that you're using RStudio to manage your GitHub repository, work through the version of the *Tidy Analysis* notebook that you've selected and sync your changes to GitHub using the `Pull -> Commit -> Push` workflow. Don't forget to knit/render your notebook so that you are syncing both the `*.Rmd` file and the resulting `*.html` file to GitHub.

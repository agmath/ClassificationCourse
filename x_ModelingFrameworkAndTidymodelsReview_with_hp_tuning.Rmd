...
+ Use *cross-validation* to tune model hyperparameters and estimate model performance.
...
## Tune Hyperparameters and Estimate Model Performance with Cross-Validation

As mentioned above, cross-validation helps us obtain more reliable estimates of future model performance. It does this by averaging performance estimates computed on each individual fold. Cross-validation can also be used to test many combinations of hyperparameters (model settings) and help us identify the optimal hyperparameter values for each model class.

Here, we'll go through creating a model specification, writing a recipe, and building a workflow for models tuning hyperparameters and those using pre-set or default hyperparameters. We'll start with a model that uses pre-set (fixed) hyperparameters, since that is simpler.

### Building a Model Workflow with Fixed Hyperparameters
...
To run *cross-validation* on a model workflow with fixed hyperparameters, we can pipe our model workflow into the `fit_resamples()` function and passing our cross-validation folds as the argument to it. 
...
### Building a Model Workflow with Tuning Hyperparameters

The only difference in fitting a workflow with tunable hyperparameters is that we should set our model hyperparameters to `tune()` rather than a fixed value.

```{r}
dt_spec <- decision_tree(max_depth = tune(),
                         min_n = tune()) %>% #build a decision tree with  max_depth fixed at 6
  set_engine("rpart") %>% #use the rpart package in R to build the tree
  set_mode("classification") #set mode to "classification" or "regression"
...
All other steps to create the workflow remain the same.

#### Tuning Hyperparameters and Estimating Model Performance with Cross-Validation

To run *cross-validation* on a model workflow in order to tune hyperparameters, we can pipe our model workflow into the `tune_grid()` function, pass our cross-validation folds as the argument to it, and then pass a grid or grid-size as parameters to this function. We'll talk much more about this throughout our semester.

```{r}
dt_cv_tune_results <- dt_wf %>%
  tune_grid(
    train_folds,
    grid = 10
    )
...
The code above would choose $10$ "space-filling" combinations of `max_depth` and `min_n` parameters for our decision tree, and would run cross-validation for each combination of hyperparameters. Once we used cross-validation to tune over our hyperparameter grid, we can collect the best-performing combinations of our hyperparameters.
